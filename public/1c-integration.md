...
Товар поддерживает `condition` (`new`/`used`), `xl` и `wheelType`. Цена может
содержать только подтвержденные `oldPrice` и `discount`. Для применяемости
обязательны сведения о происхождении и проверке: `source`, `verified`,
`verifiedAt`, `notes`. Непроверенные строки не используются как гарантия
совместимости; LLM не может подтверждать техническую применяемость.
