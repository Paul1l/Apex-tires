...
## Дополнение: каталог и локальные резервы

Схема PostgreSQL расширена миграциями 004–006: типизированные параметры шин и
дисков, внешние идентификаторы, техническая применяемость, журнал статусов
заказа и локальные резервы. 1С продолжает использовать серверный provider и
отдельные потоки products/prices/stocks; frontend не знает endpoint 1С.

CSV не может переписывать товары, которыми владеет `source_system=1c`.
Входящий `reserved` остатков трактуется как внешний резерв, **не включающий**
активные резервы заказов сайта. Сервер добавляет локальные резервы сам и
отклоняет остаток меньше их суммы. Перед реальным подключением программисты
обоих контуров обязаны согласовать передачу владения резервом при принятии
заказа в 1С: нельзя одновременно учитывать один резерв в обеих системах.
Автоматическая передача владения пока не реализована и не заявляется рабочей.

Подробности: `DATA_OWNERSHIP.md`, `PRODUCT_IMPORT.md`, `FITMENT_PROVIDER.md`.
Настройка контракта конкретной конфигурации 1С и проверка реального обмена
остаются обязательными до включения интеграции.
