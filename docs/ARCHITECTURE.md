...
## Конфигурация локального бизнеса

Публичные реквизиты, контакты, каналы связи, услуги, доставка, feature flags и
брендинг читаются только из `config/business.ts`. Источник значений —
переменные окружения. В preview отсутствующие данные скрываются или явно
помечаются как незаполненные. При
`NEXT_PUBLIC_DEPLOYMENT_STAGE=production` сборка блокируется, пока нет
критических реквизитов и каталог не переключен на PostgreSQL.

Платежи, уведомления и применяемость имеют отдельные provider-контракты.
Для email реализован адаптер Yandex Cloud Postbox; без credentials он не
выдаёт фиктивный успех. Платёжный провайдер и 1С остаются неподключёнными.
...
3. админка: защищённые роли -> CSV import/order API -> PostgreSQL;
4. OTP и сессии: `AuthService` -> `AuthRepository` -> PostgreSQL;
5. фото: CSV URL сейчас, загрузка файлов — после выбора российского object storage.

В production каталог, применяемость, зарегистрированная корзина, заказы,
согласия и заявки находятся в PostgreSQL. `localStorage` используется только
для гостевой корзины и preview-каталога.
